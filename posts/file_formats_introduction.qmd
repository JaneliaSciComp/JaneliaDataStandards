...
#### CellMap Zarr container structure

The process of reconstructing the FIB-SEM data and extracting useful information about cell organelles types classification and relative position is complex and involves many additional data manipulations. Multiple FIB-SEM image reconstructions volumes could exist at the same time, as well as multiple datasets produced by human annotators, and organelle segmentations generated by ML models. In order to properly organize and categorize datasets of different types, the [CellMap](https://www.janelia.org/project-team/cellmap) team developed a rigid [Zarr](https://zarr.dev/) group schema to store data:

```bash
.
├── <dataset name>.zarr # zarr group containing image volumes saved as zarr arrays
│	├──<reconstruction name> # group: a separate volumetric image reconstruction
│	│   ├── em # group: electron microscopy (EM) volumes
│	│   │   ├── <em volume name> # zarr group: an EM data volume
│	│   │   │   ├── s0 # zarr array: full resolution
│	│   │   │   ├── s1 # zarr array: half resolution
│	│   │   │   ┊
│	│   ├── lm # group: light microscopy (LM) volumes
│	│   │   ├── <lm volume name> # zarr group: an LM data volume
│	│   │   │   ├── s0 # zarr array: full resolution
│	│   │   │   ├── s1 # zarr array
│	│   │   │   ┊
│	│   ├── labels # zarr group: image volumes with semantics, e.g. ground-truth data, machine-learning-based
│	│   │   |  	# predictions, object segmentations, and any derived volumes.
│	│   │   ├── <inference > # zarr group: a semantic data volume
│	│   │   │   ├── <predictions> # zarr group
│	│   │   │   │   ├── <prediction volume name> # zarr array
│	│   │   │   │   │   ├── s0 # zarr array: full resolution
│	│   │   │   │   │   ├── s1 # zarr array
│	│   │   │   │   ┊
│	│   │   │   ├── <segmentations> # zarr group
│	│   │   │   │   ├── <segmentation volume name> # zarr array
│	│   │   │   │   │   ├── s0 # zarr array: full resolution
│	│   │   │   │   │   ├── s1 # zarr array
│	│   │   │   │   ┊
│	│   │   ├── <groundtruth> # zarr group: manual annotation volumes
│	│   │   │   ├── <training crop volume name> # zarr array
│	│   │   │   │   ├── <class name>
│	│   │   │   │   │   ├── s0 # zarr array: full resolution
│	│   │   │   │   │   ├── s1 # zarr array
│	│   │   │   │   ┊
```
Each Zarr container published on [openorganelle.com](https://openorganelle.janelia.org/) and stored in the [janelia-cosem-datasets](s3://janelia-cosem-datasets/) AWS S3 bucket, is structured according to this schema. Every dataset within the CellMap container is formatted according to [OME-NGFF standards](https://ngff.openmicroscopy.org/latest/) to guarantee data shareability, and a multi-scale pyramid is generated for the fast data access. 

#### Ground truth

It is beneficial to precompute certain volume information about each organelle class, present within a ground truth segment (crop), to speed up the training process of segmentation models. In the CellMap training pipeline, each ground truth is decomposed into **N** arrays of the same volume using the subgroup splitting method. **N** value is the number of classes identified as either "present in volume" or "absent in volume" during the human annotation process.

The CellMap metadata is an addition to the existing OME-NGFF metadata, stored separately within .zattrs file of "**s0**" multiscale level Zarr array, and is structured in the following way:
```bash
{
	"cellmap": {
    	"annotation": {
...
* **"annotation_type"** : "instance_segmentation"/"semantic_segmentation".
* **"encoding"** : 0 for "not present, annotated", 255 for "unknown" (not annotated) value.
* **"class_name"**: a short name of the organelle.
* **"absent"** : annotated volume value ($pixel^{3}$) where organelle class is not present
* **"unknown"** : volume that was not annotated
The absent parameter could be used to compute the volume occupied by the annotated class, to balance weights during the training of a neural network.   
